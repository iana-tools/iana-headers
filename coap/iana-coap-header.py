...
def search_and_replace_c_typedef_enum(document_content, typename, head_comment, c_enum_list):
    # Generate enumeration header content
    c_enum_content = generate_c_enum_content(head_comment, c_enum_list)
...
def iana_coap_content_formats_existing_header_update(coap_content_content_format, c_typedef_name: str):
    existing_enum_name = {}
    with open(iana_coap_c_header_file_path, 'r') as file:
        c_header_content = file.read()
        existing_enum_name = extract_enum_values_from_c_code(c_header_content, c_typedef_name)
...
def iana_coap_content_formats_list_to_c_enum_list(coap_content_content_format):
    c_enum_list = {}
...
def iana_coap_content_formats_c_typedef_enum_update(header_file_content: str) -> str:
    ...
    source_name = iana_coap_content_format_settings.get("name", "")
    source_url = iana_coap_content_format_settings.get("source", "")
    ...
    coap_content_content_format = iana_coap_content_formats_parse_csv(csv_content)

    # Check for existing enum so we do not break it
    coap_content_content_format = iana_coap_content_formats_existing_header_update(coap_content_content_format, iana_coap_content_format_settings["c_typedef_name"])

    # Format to enum name, value and list
    c_enum_list = iana_coap_content_formats_list_to_c_enum_list(coap_content_content_format)
    ...
    return search_and_replace_c_typedef_enum(header_file_content, c_typedef_name, c_head_comment, c_enum_list)
